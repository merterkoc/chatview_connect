...
final class ChatViewFireStoreDatabase implements DatabaseService {
  ...
  Future<void> updateChatRoomUserMetadata({
    ...
    return ChatViewFireStoreCollections.chatParticipantsCollection(
      _chatRoomCollectionPath(chatId),
    ...
    try {
      ...
      switch (FirestoreExceptionType.fromCode(exception.code)) {
        case FirestoreExceptionType.notFound:
          return _setUserActiveStatus(id: userId, status: userStatus);
  ...
  Future<String?> createOneToOneChat({
    ...
    final isUsersExists = await Future.wait([
      _doesUserExists(userId),
      _doesUserExists(otherUserId),
    ]);
  ...
  Future<String?> findOneToOneChatRoom({
    ...
    try {
      ...
      return _doesChatExists(otherUserId: otherUserId, userId: userId);
    } catch (_) {
      return null;
    }
  }
